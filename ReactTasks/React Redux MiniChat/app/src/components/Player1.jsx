...
import { setPlayer1Message } from './redux/reducers/player1';
...
const Player1 = () => {
    ...
    const SentMessage = (e) => {
        e.preventDefault()
        dispatch(setPlayer1Message(inputValue.current.value))
        if (inputValue.current.value && location.pathname !== "/player-2") {
            dispatch(setMessage2Count(messageP2Count + 1))
        } else {
            ...
            <form className='p-10 bg-green-400 rounded-md'>
                {
                    message1Value.map((e) => {
                        return (
                            <p key={shorId.generate()} className='text-yellow-500 text-xl text-right bg-gray-300 rounded-md my-2 p-2' >{name1Value + " : " + e}</p>
                        )
                    })
                }
                {
                    message2Value.map((e) => {
                        return (
                            <p key={shorId.generate()} className='text-yellow-500 text-xl bg-gray-300 rounded-md my-2 p-2' >{name2Value + " : " + e}</p>
                        )
                    })
                }
